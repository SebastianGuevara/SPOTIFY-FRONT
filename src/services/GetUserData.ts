...
export const getUserDataById = async (accessToken: String, refreshToken: String, userId: String) =>{
    const myHeaders = new Headers();
    myHeaders.append("Authorization", "Bearer "+accessToken);
    ...
    return fetch(`https://api.spotify.com/v1/users/${userId}`, requestOptions)
    .then(response => response.text())
    .then(result => {
        if(JSON.parse(result).error){
            if("The access token expired"===JSON.parse(result).error.message){
